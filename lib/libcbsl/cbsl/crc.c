...
uint16_t getCRC16(uint8_t *startaddr,uint32_t size)
{
  ...
  {
    ret_crc = CRC_TABLE[(uint8_t) (*pflash) ^ (uint8_t) ret_crc] ^ (uint8_t) (ret_crc >> 8);
    pflash++;
  }
...
uint16_t getCRC16_from_file(FILE *fp, uint32_t size)
{
  ...
  for (ctr=0; ctr < size ; ctr++)
  {
    ret_crc = CRC_TABLE[pflash ^ (uint8_t) ret_crc] ^ (uint8_t) (ret_crc >> 8);
    fread(&pflash,1,1,fp);
  }
